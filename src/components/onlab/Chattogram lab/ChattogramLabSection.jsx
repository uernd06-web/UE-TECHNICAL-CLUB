...
import ChattogramCard from "./ChattogramCard";

const ChattogramLabSection = ({ ctgData }) => {
    ...
    <section className="max-w-7xl mx-auto my-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-red-600/30 p-6 sm:p-8 md:p-10 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {ctgData?.map((lab) => (
            <ChattogramCard key={lab.id || lab.title} lab={lab} />
          ))}
        </div>
...
export default ChattogramLabSection;