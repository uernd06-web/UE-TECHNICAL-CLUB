...
const MentorsSection = ({mentors}) => {
  console.log(mentors)
    const [selectedMentors, setSelectedMentors]=useState(null)
    const [isModalOpen,setIsModalOpen]=useState(false)
    const handleOpenModal=(mentors)=>{
        setSelectedMentors(mentors)
        setIsModalOpen(true)
    }
    const handleCloseModal= ()=>{
        setIsModalOpen(false)
        setSelectedMentors(null)
    }
  return (
    <div>
            ...
            >
              ...
              <div className="relative bg-[#0d0d0d] pt-8 px-6 pb-0 flex flex-col items-center justify-end overflow-hidden h-[320px]">
                ...
                <div className="absolute top-4 right-4 z-10 bg-black/60 backdrop-blur-md border border-white/10 text-white text-xs font-bold px-3 py-1 rounded-full">
                  {mentor.initials || mentor.initials.match(/\(([^)]+)\)/)?.[1] || "MENTOR"}
                </div>
      ...
      <MentorsModal isOpen={isModalOpen} onClose={handleCloseModal} mentor={selectedMentors}></MentorsModal>
    </section>
    </div>
  )
}
